...
static NSMutableDictionary *textureHash;
                                    ...
                                    metalKitTextureLoader:(nonnull MTKTextureLoader *)textureLoader
{
    ...
    for (MDLMaterialProperty *property in propertiesWithSemantic)
    {
           ...
           property.type == MDLMaterialPropertyTypeURL)
        {
            ...
            if ([textureHash objectForKey:URLString]) {
                _colorTexture = [textureHash objectForKey:URLString];
                break;
            }
            ...
            if(texture) {
                // ...return it
                [textureHash setObject:texture forKey:URLString];
            ...
            if (textureHash[lastComponent]) {
                _colorTexture = textureHash[lastComponent];
                break;
            }
            ...
            if(texture) {
                // ...return it
                [textureHash setObject:texture forKey:lastComponent];
                _colorTexture = texture;
                break;
            }
            
            UIImage *assetTexture = [UIImage imageNamed:lastComponent];
            texture = [textureLoader newTextureWithCGImage:assetTexture.CGImage options:nil error:nil];
            if (texture) {
                [textureHash setObject:texture forKey:lastComponent];
                _colorTexture = texture;
                break;
            ...
            NSLog(@"Requested material property semantic: %lu string: %@",materialSemantic, property.stringValue);
...
//                                materialSemantic, property.stringValue];
            ...
            _color = property.float3Value;
            break;
        } else if (property.type == MDLMaterialPropertyTypeFloat2) {
            _color = simd_make_float3(property.float2Value.x, property.float2Value.y, 0);
            break;
        } else {
            NSLog(@"Requested material property semantic: %lu, type = %lu", materialSemantic, (unsigned long)property.type);
        }
    }
...
//    NSLog(@"Requested material property semantic: %lu propertiesWithSemantic :%lu", materialSemantic, (unsigned long)propertiesWithSemantic.count);
}
...
//        }
        ...
        [self setupMetalTextureFromMaterial:modelIOSubmesh.material
